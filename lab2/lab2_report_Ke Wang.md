...
The first challenge is to make and build the Theseus which is always onerous and error-prone in C/C++. The dependencies versions or something are always making troubles. However, it seems rather easy to make and build the Theseus system writen in Rust. Actually, I successed in building the Theseus at my first try, which should be unbelievable for building systems writen in C/C++.
